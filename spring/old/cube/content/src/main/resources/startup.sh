...
checkPermissions() {
  touch "$pid_file" &>/dev/null || {
    echoRed "Operation not permitted (cannot access pid file)"
    return 4
  }
  touch "$log_file" &>/dev/null || {
    echoRed "Operation not permitted (cannot access log file)"
    return 4
  }
}
...
pid_file="$PID_FOLDER/$PID_FILENAME"
...
stop() {
  ...
  pushd "$working_dir" >/dev/null
  [[ -f $pid_file ]] || {
    echoYellow "Not running (pidfile not found)"
    return 0
  }
  pid=$(cat "$pid_file")
  isRunning "$pid" || {
...
force_reload() {
  ...
  pushd "$working_dir" >/dev/null
  [[ -f $pid_file ]] || {
    echoRed "Not running (pidfile not found)"
    return 7
  }
  pid=$(cat "$pid_file")
  rm -f "$pid_file"
  isRunning "$pid" || {
    echoRed "Not running (process ${pid} not found)"
    return 7
  }
  do_stop "$pid" "$pid_file"
  do_start
